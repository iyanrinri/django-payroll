...
export default function OvertimeList({refreshKey}) {
    ...
    const showConfirmation = (item) => {
        setSelectedItem(item);
        setMessage(`Are you sure you want to delete the item for: ${item.clock_date_display} with overtime: "${item.hours} hours" ?`);
        setConfirmationDelete(true);
    }

    const handleDelete = async () => {
        try {
            const response = await fetch(`/api/overtimes/${selectedItem.id}`, {
                method: 'DELETE',
                headers: {
    ...
    const fetchItems = async () => {
        setLoading(true);
        try {
            const response = await fetch(`/api/overtimes?page=${page}`, {
                method: 'GET',
                headers: {
                    'Accept': 'application/json',
                },
    ...
    return (
        <div className="relative overflow-x-auto">
            {showFormModal && (
                <OvertimeForm item={selectedItem} onClose={() => setShowFormModal(false)} onSubmit={onSubmit} />
            )}
            {confirmationDelete && (
                    ...
                    {itemData && itemData.results.map((item, index) => (
                        <tr key={index}
                            ...
                            <td className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                {item.clock_date_display || 'N/A'}
                            </td>
                            <td className="px-6 py-4">{item.hours || 'N/A'}</td>
